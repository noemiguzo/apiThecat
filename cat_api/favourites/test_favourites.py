...
class TestImages:
    ...
    def test_post_favourite(self, log_test_names):
        """
        Test post favourite
        """
        LOGGER.info("Test post favourite")
        URL_CAT_API_FAVOURITES = f"{URL_CATAPI}/favourites"
        body_favourite = {
        ...
        response = self.rest_post_client.request("post", URL_CAT_API_FAVOURITES, data=json.dumps(body_favourite))

        LOGGER.info("Test post favourite response", response)
        if response["status_code"] == 200:
            self.favourite_list.append(response["body"]["id"])
        self.validate.validate_response(response, "favourites", "create_favourite")

    @pytest.mark.acceptance
    def test_delete_favourite(self, create_a_favourite, log_test_names):
        """
            delete favourite
    ...
    @pytest.mark.acceptance
    def test_get_favourite(self, create_a_favourite, log_test_names):
        """
    ...
    def test_image_id_is_required_create_favourite(self):
        ...
        self.rest_post_client = RestClient()
        self.rest_post_client.session.headers.update(self.header_post)
        response = self.rest_post_client.request("post", self.URL_CAT_API_FAVOURITES, data=json.dumps(body_favourite))
